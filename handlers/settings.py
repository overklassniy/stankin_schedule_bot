...
async def can_manage_settings(bot: Bot, user_id: int, chat_id: int, chat_type: str) -> bool:
    ...
    if await is_admin(user_id):
        logger.debug("can_manage_settings: user_id=%s is global admin", user_id)
        return True
    if chat_type not in ("group", "supergroup"):
        logger.debug("can_manage_settings: user_id=%s chat_type=%s not group/supergroup", user_id, chat_type)
        return False
    try:
        member = await bot.get_chat_member(chat_id, user_id)
        ok = member.status in ("creator", "administrator")
        logger.debug("can_manage_settings: user_id=%s chat_id=%s status=%s -> %s", user_id, chat_id, member.status, ok)
        return ok
    except Exception as e:
        logger.debug("can_manage_settings: get_chat_member failed user_id=%s chat_id=%s: %s", user_id, chat_id, e)
        return False
...
async def handle_settings_command(message: types.Message, state: FSMContext, bot: Bot) -> None:
    ...
    """
    logger.debug("settings command from user_id=%s chat_id=%s", message.from_user.id, message.chat.id)
    await state.clear()
    if not await can_manage_settings(bot, message.from_user.id, message.chat.id, message.chat.type):
        logger.debug("User %s denied settings (not admin) in chat %s", message.from_user.id, message.chat.id)
        return
    thread_id = getattr(message, "message_thread_id", None)
    group = await resolve_group(message.chat.id, thread_id)
    if not group:
        logger.info("User %s opened settings in unattached chat %s", message.from_user.id, message.chat.id)
        await message.answer("Сначала добавьте бота в группу – настройки работают только в привязанных группах.")
        return
    logger.info("Settings opened for group id=%s chat_id=%s by user_id=%s", group["id"], message.chat.id, message.from_user.id)
...
async def handle_settopic_command(message: types.Message, state: FSMContext, bot: Bot) -> None:
    ...
    """
    logger.debug("settopic command from user_id=%s chat_id=%s", message.from_user.id, message.chat.id)
    await state.clear()
    if not await can_manage_settings(bot, message.from_user.id, message.chat.id, message.chat.type):
        logger.debug("settopic: user_id=%s denied (not admin)", message.from_user.id)
        return
    group = await get_group_by_chat(message.chat.id)
    if not group:
        logger.info("settopic: no group for chat_id=%s", message.chat.id)
        await message.answer("Сначала добавьте бота в группу.")
        return
    thread_id = getattr(message, "message_thread_id", None)
    await update_group_settings(group["id"], thread_id=thread_id)
    logger.info("Topic set for group id=%s chat_id=%s thread_id=%s", group["id"], message.chat.id, thread_id)
...
async def settings_receive_time(message: types.Message, state: FSMContext) -> None:
    ...
    logger.debug("settings_receive_time from user_id=%s text=%s", message.from_user.id, message.text)
    data = await state.get_data()
    gid = data.get("settings_group_id")
    if not gid:
        logger.debug("settings_receive_time: no settings_group_id in state, clearing")
        await state.clear()
        return
    ...
    if not parts or len(parts) != 2:
        logger.debug("settings_receive_time: invalid format gid=%s raw=%s", gid, raw)
    ...
    try:
        ...
        logger.debug("settings_receive_time: not numbers gid=%s raw=%s", gid, raw)
        await message.answer("Введите время в формате ЧЧ:ММ, например 07:30.")
        return
    if not (0 <= hour <= 23) or not (0 <= minute <= 59):
        logger.debug("settings_receive_time: out of range gid=%s hour=%s minute=%s", gid, hour, minute)
        await message.answer("Часы – от 0 до 23, минуты – от 0 до 59.")
        return
    await state.clear()
    await update_group_settings(gid, send_hour=hour, send_minute=minute)
    logger.info("Group id=%s: send time set to %s:%s", gid, hour, minute)
...
async def settings_receive_group_code(message: types.Message, state: FSMContext) -> None:
    ...
    logger.debug("settings_receive_group_code from user_id=%s text=%s", message.from_user.id, message.text)
    data = await state.get_data()
    gid = data.get("settings_group_id")
    if not gid:
        logger.debug("settings_receive_group_code: no settings_group_id in state, clearing")
        await state.clear()
        return
    raw = (message.text or "").strip()
    if not raw:
        logger.debug("settings_receive_group_code: empty input gid=%s", gid)
    ...
    logger.info("Group id=%s: schedule source (group code) set to %s", gid, raw)
...
async def settings_callback_group_code(call: CallbackQuery, state: FSMContext) -> None:
    ...
    logger.debug("callback settings_group_code from user_id=%s chat_id=%s", call.from_user.id, call.message.chat.id)
    if not await can_manage_settings(call.bot, call.from_user.id, call.message.chat.id, call.message.chat.type):
        logger.debug("callback settings_group_code: denied")
        await call.answer("Недостаточно прав.", show_alert=True)
        return
    thread_id = getattr(call.message, "message_thread_id", None)
    group = await resolve_group(call.message.chat.id, thread_id)
    if not group:
        logger.info("callback settings_group_code: group not found chat_id=%s", call.message.chat.id)
    ...
    logger.debug("callback settings_group_code: FSM wait_group_code for group id=%s", group["id"])
...
async def settings_callback_time(call: CallbackQuery, state: FSMContext) -> None:
    ...
    logger.debug("callback settings_time from user_id=%s chat_id=%s", call.from_user.id, call.message.chat.id)
    if not await can_manage_settings(call.bot, call.from_user.id, call.message.chat.id, call.message.chat.type):
        await call.answer("Недостаточно прав.", show_alert=True)
        return
    thread_id = getattr(call.message, "message_thread_id", None)
...
async def settings_callback_image(call: CallbackQuery) -> None:
    ...
    logger.debug("callback settings_image from user_id=%s chat_id=%s", call.from_user.id, call.message.chat.id)
    if not await can_manage_settings(call.bot, call.from_user.id, call.message.chat.id, call.message.chat.type):
        await call.answer("Недостаточно прав.", show_alert=True)
        return
    thread_id = getattr(call.message, "message_thread_id", None)
    ...
    logger.info("Group id=%s: enable_image=%s", group["id"], new_val)
...
async def settings_callback_tomorrow(call: CallbackQuery) -> None:
    ...
    logger.debug("callback settings_tomorrow from user_id=%s chat_id=%s", call.from_user.id, call.message.chat.id)
    if not await can_manage_settings(call.bot, call.from_user.id, call.message.chat.id, call.message.chat.type):
        await call.answer("Недостаточно прав.", show_alert=True)
        return
    thread_id = getattr(call.message, "message_thread_id", None)
    ...
    logger.info("Group id=%s: enable_tomorrow_button=%s", group["id"], new_val)
    await call.message.edit_reply_markup(reply_markup=_settings_inline_keyboard(group))
...
async def settings_callback_test_load(call: CallbackQuery) -> None:
    ...
    logger.debug("callback settings_test_load from user_id=%s chat_id=%s", call.from_user.id, call.message.chat.id)
    if not await can_manage_settings(call.bot, call.from_user.id, call.message.chat.id, call.message.chat.type):
        await call.answer("Недостаточно прав.", show_alert=True)
        return
    thread_id = getattr(call.message, "message_thread_id", None)
    ...
    if not group_code:
        logger.debug("callback settings_test_load: no group code for group id=%s", group["id"])
        await call.answer()
    ...
    try:
        ...
        logger.exception("Test load failed for group_code=%s", group_code)
